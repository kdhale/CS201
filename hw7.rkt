...
(define (ok-string? value)
  (if (not (list? value))
     ...
     (cond
       [(empty? value) #t]
       [(symbol? (car value))(and #t (ok-string? (cdr value)))]
       [else #f])))
...
(define (flip bias)
  (if (> (random) bias) ;random returns random number from 1-0, so check if number greater than bias, if so then must be false
      #f
      #t))
...
(define (pick lst)
  (if (empty? lst)
      #f
      (list-ref lst (random 0 (length lst))))); generate random number between 0- length of list, then return value at that index
...
(define (generate-string-from-reg-exp exp)
  (let ([lststrs (make-string-lst exp)])
    (flatten lststrs)))

(define (make-string-lst exp)
  (cond
    [(symbol? exp) (list exp)]
    [(concat? exp)(append (make-string-lst (concat-arg1 exp)) (make-string-lst(concat-arg2 exp)))]
    [(either? exp)(pick (append (list (make-string-lst (either-arg1 exp))) (list (make-string-lst(either-arg2 exp)))))]
    [(repeat? exp)(if (flip .8)
                      (append (list (make-string-lst (repeat-arg1 exp))) (list (make-string-lst exp)))
                      (make-string-lst (repeat-arg1 exp)))]
    [else (list exp)]))
...
(define (dfa-accepts? mach str)
  (cond
    [(and (empty? str)(member? (dfa-start-state mach) (dfa-accepting-states mach))) #t] ; if it's empty, then true because already at starting state
    [else (change-states str (dfa-start-state mach) mach)]))

(define (change-states str state mach)
  (let ([table (dfa-transitions mach)])
  (cond
    [(empty? str) (member? state (dfa-accepting-states mach))]; if the string is empty, return whether the current state equals the appecting-state
    [(not (boolean? (lookup (list state (car str)) table)))(change-states (cdr str)(lookup (list state (car str)) table) mach)] ; if looking up '((car str) state) returns not #f, then chenge states to the next state found by lookup, and run with next value in str
    [else #f])))                      
(define (lookup key table) ; finds a value in an entry-key list, else returns false
  (cond
    [(empty? table) #f]
...
;dfa (alphabet states start-state accepting-states transitions) #:transparent
;************************************************************
...
(define (list-leaf-labels tree)
  (cond
    ...
    [else (list (leaf-label tree))])); leaf
...
(define (generate-parse-tree-from-cfg grammar)
  (node (cfg-start-symbol grammar) (cfg-recurse grammar (pick (generate-rules-lst (cfg-rules grammar) (cfg-start-symbol grammar))))))

(define (generate-rules-lst rules start) ; makes a list of all rule options that a lhs has
  (cond
    [(empty? rules) '()]
    [(equal? start (rule-lhs (car rules)))(cons (rule-rhs (car rules)) (generate-rules-lst (cdr rules) start))]; if the lhs or the current rule equals start, add it's list of elements to the list
    [else (generate-rules-lst (cdr rules) start)]))

(define (cfg-recurse grammar lst)
  (cond
    [(empty? lst)'()]
    [(member? (car lst)(cfg-nonterminals grammar))(cons (node (car lst) (cfg-recurse grammar (pick (generate-rules-lst (cfg-rules grammar) (car lst)))))(cfg-recurse grammar (cdr lst)))]
    [(member? (car lst)(cfg-terminals grammar))(cons (leaf (car lst))(cfg-recurse grammar (cdr lst)))]
    [else #f]))

(define (member? var lst); if var is member of lst, will return a lst, therefore true
  (list? (member var lst)))
...
(define exp-mcd
  (let* ([np (either (concat (either '(a) '(the)) (either '(mouse) (either '(cat) '(dog)))) '(it))]
         [vp-end (either (either '(swam) '(slept)) (concat (either '(chased) '(evaded)) np))]
         [vp (either (either '(swam) '(slept)) (either (concat (either '(chased) '(evaded)) np) (repeat (concat (either '(believed) '(dreamed)) (concat '(that) (concat np vp-end))))))])
   (concat
    (either (concat (either '(a) '(the)) (either '(mouse) (either '(cat) '(dog)))) '(it))
    (either (either '(swam) '(slept)) (either (concat (either '(chased) '(evaded)) np) (concat (either '(believed) '(dreamed)) (concat '(that) (concat np vp-end)) ))))))
...
(define my-cfg
  (cfg
   '(a boy girl kid and slept swam ate poked dreamed believed that pomegranate)
   '(s np vp det n pn vi vt v3)
   's
   (list
    (rule 's '(np vp))
